...
import uvicorn
# ⚠️ 确保 requirements.txt 里是 moviepy==1.0.3
...
# 👇 强制写死 Render 域名 (保持你之前的配置)
PUBLIC_BASE_URL = "https://cheng-lan-aidao-yan.onrender.com"
print(f"DEBUG: 🚀 系统已强制锁定公网地址 -> [{PUBLIC_BASE_URL}]")
        ...
        try:
            ...
            for kf in StoryEngine.KEYFRAMES[:2]: 
                async with self.sem_process:
                    ...
                    # 1. Gemini (不再依赖 brand/product 变量)
...
# --- 路由 (已删除 brand/product 参数) ---
@app.post("/update_playlist")
async def update_playlist(
    bg_tasks: BackgroundTasks,
    # 删除了 brand 和 product 的 Form 参数
